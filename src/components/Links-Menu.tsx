...
export default function Links({ text, link, iconName, WebRoute }: { WebRoute: string, text: string, link: string, iconName: string }) {
  ...
  return (
    <Link href={link}>
    <li className={`flex gap-3 items-center hover:bg-[#888888] hover:text-white transition cursor-pointer p-2 rounded-full ${route === WebRoute || route === '/dashboard' ? 'bg-[#0980B4] text-white font-bold' : 'text-[#888888]'}`}>
